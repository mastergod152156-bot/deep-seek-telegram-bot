...
# Configuration from environment variables (for cloud)
TELEGRAM_BOT_TOKEN = os.getenv('TELEGRAM_BOT_TOKEN', '8586547665:AAGCy168pxPBcRhseoekXK4T_etV4DQPhyU')
DEEPSEEK_API_KEY = os.getenv('DEEPSEEK_API_KEY', 'sk-51e64051d8b74472936d2c8c60ea4bec')
...
# Store authorized users (in production, use database)
        ...
        try:
            payload = {
                ...
                "messages": [
                    {
                        "role": "system", 
                        "content": "You are a helpful assistant that provides clear and concise responses in the same language as the user."
                    },
                    {
                        "role": "user",
                        "content": question
...
**အသုံးပြုနည်းများ:**
• မေးခွန်းတစ်ခုခုမေးပါ
• /help - အကူအညီရယူရန်
• /ask [မေးခွန်း] - AI ကိုမေးမြန်းရန်
...
/unauthorize [user_id] 
/list_users
/restart - Bot restart (Admin only)
    """
...
🔄 **Restart လုပ်ခဲ့သည့်အကြိမ်:** {bot_stats['restart_count']}
🌐 **Server:** Cloud Hosted (24/7)
    """
...
async def ask_question(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_to_deepseek(update, question)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_to_deepseek(update, message_text)

async def send_to_deepseek(update: Update, question: str):
...
async def restart_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Bot restart initiated by admin {user_id}")
